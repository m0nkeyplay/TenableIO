...
# 	file Date: 	2019-04-4
#
# 	purpose: 	Queue Up Scan Data for download based on hostnames from the lastest scan based on scan name
#
...
hello = '##########################################################################\n'
hello +='#                                                                        #\n'
hello +='#                     Tenable IO Hostname Export - Queue Stage           #\n'
...
def create_search(file):
    ...
    for plugin in pluginFile:
        line = plugin.strip()
        searchFor += '"filter.'+str(x)+'.filter":"host.hostname","filter.'+str(x)+'.quality":"match","filter.'+str(x)+'.value":"'+line+'",'
        x += 1
    pluginFile.close()
    return searchFor
